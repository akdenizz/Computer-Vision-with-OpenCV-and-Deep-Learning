# -*- coding: utf-8 -*-
"""
Created on Sun Feb 14 13:48:29 2021
...
OPENCV_OBJECT_TRACKERS = {"csrt"      : cv2.TrackerCSRT_create,
		                  "kcf"       : cv2.TrackerKCF_create,#kernelized correlation filters
		                  "boosting"  : cv2.TrackerBoosting_create,
		                  "mil"       : cv2.TrackerMIL_create,#multiple instance learning
		                  "tld"       : cv2.TrackerTLD_create, #tracking, learning and detection
		                  "medianflow": cv2.TrackerMedianFlow_create,
		                  "mosse"     : cv2.TrackerMOSSE_create}#minimum output sum of squared error

tracker_name = "mil"
tracker = OPENCV_OBJECT_TRACKERS[tracker_name]()
print("Tracker:", tracker_name)

#upload the ground truth
...
track_list = []
start_time = time.time() #like counter
...
while True:
    ...
    if ret:
        ...
        if key == ord("t"):
            ...
            initBB = cv2.selectROI("Frame", frame, fromCenter = False)#RegionOfInterest -- region that i want to track
...
if len(track_df) != 0:
    ...
    plt.xlabel("frame")
    plt.ylabel("Euclidean Distance between ground truth and track")
